...
use std::fs;
use crate::simd::{SimdProcessor, SimdChecksum};
...
impl PerformanceBenchmark {
    ...
    pub fn generate_test_files(&mut self, base_path: &str) -> Result<(), Box<dyn std::error::Error>> {
        let sizes = vec![
            (1024, "1KB"),
            (1024 * 1024, "1MB"),
            (10 * 1024 * 1024, "10MB"),
    ...
    pub fn benchmark_zero_copy(&mut self, test_file: &TestFile) -> Result<BenchmarkResult, Box<dyn std::error::Error>> {
        let start = Instant::now();
        
        // Test zero-copy reading
        let mut reader = ZeroCopyReader::new(&test_file.path)?;
        let mut total_read = 0;
        
        while let Some(chunk) = reader.read_chunk(64 * 1024)? {
    ...
    pub fn benchmark_rsync(&self, source: &str, dest: &str) -> Result<BenchmarkResult, Box<dyn std::error::Error>> {
        let start = Instant::now();
    ...
    pub fn benchmark_scp(&self, source: &str, dest: &str) -> Result<BenchmarkResult, Box<dyn std::error::Error>> {
        let start = Instant::now();
    ...
    pub fn run_full_benchmark(&mut self, test_dir: &str) -> Result<Vec<BenchmarkResult>, Box<dyn std::error::Error>> {
    ...
    pub fn print_results(&self) {
        ...
        for result in &self.results {
            ...
            println!("{:<25} {:<10} {:<12.2?} {:<15.2} MB/s {:<15.1} {:<15.1}",
                     result.operation,
                     size_str,
                     result.duration,
        ...
        if let Some(fastest) = self.results.iter().max_by(|a, b| 
            ...
            println!("Fastest operation: {} at {:.2} MB/s", 
